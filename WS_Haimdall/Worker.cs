...
namespace WS_Haimdall
{
    public class Worker : BackgroundService
    {
            ...
            if (_opcSession != null)
            {
                ...
                try
                {
                        ...
                        var writes = new WriteValueCollection()
                    {
                        ...
                        {
                            Log.Information($"Parameter Write to PLC -Serial No :{serial} | TestValue : {data.TestValue} | High Toll : {data.TolHi} | Set Value : {data.TolOk} | Low Toll : {data.TolLo} | Test Date : {data.TestDate} | Status : {data.Status}");
        ...
        private async void Tmr_Elapsed(object? sender, ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                if (!Directory.Exists(Config.WatchFolder))
                {
                    ...
                    Log.Error("Network path NOT accessible.");
                    return;
                }
                else
                {
                    await NewMethod(Config.Node_CheckMapDrive, true);
                }
                #endregion
        ...
        private async Task NewMethod(string NodeId, bool val)
        {
            try
            {
                var ctd = new CancellationTokenSource();
                ...
                WriteResponse writeResponse = await _opcSession.WriteAsync(null, writeValue, token);

                writeResponse.Results.All(code => code == StatusCodes.Good);

            }
            catch (Exception ex)
            {
                Log.Error($"NewMethod  {NodeId}" + ex.Message);
            }
        }
    }
    ...
    public class TestData
    {
        ...
        public double TolHi { get; set; }
        public double TolOk { get; set; }
        public double TolLo { get; set; }
    ...
    public class FileParser
    {
        public static TestData Parse(string file)
        {
            try
            {
                ...
                double testValue = GetDouble(dict, "Test_Value");
                double tolHi = GetDouble(dict, "TOL_HI");
                double tolOk = GetDouble(dict, "TOL_OK");
                double tolLo = GetDouble(dict, "TOL_LO");
        ...
        private static double GetDouble(Dictionary<string, string> dict, string key)
        {
            //return dict.ContainsKey(key) && double.TryParse(dict[key], out double val)
            //    ? val
            //    : 0;

            if (dict.TryGetValue(key, out string valStr) &&
